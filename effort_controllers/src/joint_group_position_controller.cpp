...
controller_interface::InterfaceConfiguration
JointGroupPositionController::command_interface_configuration() const
{
  ...
  for (const auto & joint : joint_names_) {
    command_interface_config.names.push_back(joint + "/effort");
  }
...
{
  auto period_chrono = std::chrono::duration_cast<std::chrono::nanoseconds>(
